...
SouvenirData::SouvenirData() {
	std::ifstream inFile;
	inFile.open("souvenirInfoList.txt");
...
void SouvenirData::updateSouvenirList() {
    ...
    outFile.open("souvenirInfoList.txt");
    ...
    for(; it != listOfSouvenirs.end(); it++, endFileCheck++) {
        ...
        if(endFileCheck != listOfSouvenirs.end()) outFile << std::endl;
    }//end - for
...
std::string SouvenirData::getAllInfoAt(std::string stadiumName) {
	//the .count() method will return a bool if the key is in the map.
	//if stadium is not in the map, return false.
	...
	returnString << stadiumName << "\n";

	std::vector<souvenirInfo>::iterator it = listOfSouvenirs.at(stadiumName).begin();
	for(;it != listOfSouvenirs.at(stadiumName).end(); it++) {
		returnString << it->item << "\n";
		returnString << it->price << "\n";
	}//end - while
